...
- `generateTotpCode(secret: string): Promise<string>` — generates the current 6-digit TOTP code from a Base32 secret using `otplib`; waits for the next 30-second window when the current one is about to expire or its code was already used
...
- **`loginViaUi`** — an auto fixture (`{ auto: true, timeout: 60_000 }`) that runs before every test: it calls `loginPage.goTo()` then `loginPage.login()` (which defaults to the demo credentials from `@helpers/users`). Tests do not destructure or reference it; authentication just happens. Specs that need the login form itself opt out with `test.use({ skipAutoLogin: true })`.
- **Two-factor login** — the demo client account has TOTP 2FA enabled. `login()` submits username and password, waits for the MFA step, generates the current code with `generateTotpCode()` and submits it, then waits for the post-login dashboard element, so the session is established before the test body runs. The longer fixture timeout covers waiting for a fresh 30-second TOTP window.
...
- `DEMO_USER_CLIENT_TOTP_SECRET_KEY` — Base32 TOTP secret of the demo client account (the "Manual setup key" shown when enabling 2FA on `/profile`); never commit it, `config/.env.local` is git-ignored
